...
API_URL = os.environ.get("API_URL", "http://localhost:8000/v1/messages")
API_FORMAT = os.environ.get("API_FORMAT", "anthropic")  # "anthropic" or "openai"

BASE_DIR = "/Users/talon/.openclaw/workspace/reasoning-data"
JSONL_PATH = os.environ.get("JSONL_PATH", os.path.join(BASE_DIR, "reasoning-train.jsonl"))
...
def call_llm(prompt):
    ...
    if API_FORMAT == "anthropic":
        ...
        payload = {
            "model": MODEL,
            "max_tokens": 8000,
            "system": "You are a synthetic data generator. You output ONLY XML-formatted logic puzzles. You never use real-world names or concepts. You follow structural constraints exactly.",
            "messages": [
        ...
        payload = {
            "model": MODEL,
            "messages": [
                {"role": "system", "content": "You are a synthetic data generator. You output ONLY XML-formatted logic puzzles. You never use real-world names or concepts. You follow structural constraints exactly."},
                {"role": "user", "content": prompt}
            ],
            "max_tokens": 32000,
            "temperature": 0.95,
        ...
        if API_FORMAT == "anthropic":
            if "content" in data:
                ...
                # If content is a list of blocks
                    ...
                    text_parts = [b.get("text", "") for b in data["content"] if isinstance(b, dict)]
                    return "\n".join(text_parts) if text_parts else data.get("completion", "")
            # Fallback to older anthropic complete format
            if "completion" in data:
                return data["completion"]
            raise KeyError("No content or completion found")
        else:
...
def main():
    ...
    while True:
        ...
        # Short break to avoid aggressive bursting
        import random
